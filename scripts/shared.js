...
const testimonialsSection = document.querySelector(".testimonials"),
  ...
  testimonialsTrack = document.querySelector(".testimonials__grid");
if (testimonialsSection && testimonialsSlider && testimonialsTrack) {
  const e = Array.from(testimonialsTrack.querySelectorAll(".testimonial")),
    t = window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches;
  testimonialsSlider.tabIndex = 0;
  let o = 0,
    r = 0,
    s = 0;
  const a = (e, t, o) => Math.min(o, Math.max(t, e)),
    n = () => {
      const e = window.innerWidth,
        t = window.innerHeight,
        o = t <= 500 && e > t;
      return e >= 1600 && !o ? 4 : e <= 480 || o ? 1.5 : e <= 980 ? 2 : 3;
    },
    i = () => {
      if (!e.length) return;
      const t = e[0],
        i = e[1];
      r = i ? i.offsetLeft - t.offsetLeft : t.getBoundingClientRect().width;
      const c = n();
      ((s = Math.max(0, e.length - Math.floor(c))), (o = a(o, 0, s)));
    },
    c = ({ animate: t = !0 } = {}) => {
      if (!e.length) return;
      t || (testimonialsTrack.style.transition = "none");
      const s = -o * r;
      ((testimonialsTrack.style.transform = `translate3d(${s}px, 0, 0)`),
        t ||
          (testimonialsTrack.offsetHeight,
          (testimonialsTrack.style.transition = "")));
    },
    d = (e, { animate: t = !0 } = {}) => {
      ((o = a(e, 0, s)), c({ animate: t }));
    };
  (i(), c({ animate: !1 }));
  let l = 0;
    ...
    _ = (t) => {
      ("mouse" === t.pointerType && 0 !== t.button) ||
        (e.length &&
          ...
          (w = 0),
          (b = Date.now()),
          testimonialsSlider.setPointerCapture?.(t.pointerId)));
    },
    y = (e) => {
      if (!m) return;
      const t = Date.now(),
      ...
      g ||
        (g = requestAnimationFrame(() => {
          ...
          let t = u + e;
          const o = -s * r;
          (t > 0 ? (t *= 0.3) : t < o && (t = o + 0.3 * (t - o)),
            (testimonialsTrack.style.transform = `translate3d(${t}px, 0, 0)`));
        }));
    },
    L = () => {
      if (!m) return;
  ...
  (testimonialsSlider.addEventListener("pointerdown", _),
    testimonialsSlider.addEventListener("pointermove", y),
    testimonialsSlider.addEventListener("pointerup", L),
    testimonialsSlider.addEventListener("pointercancel", L),
    testimonialsSlider.addEventListener("lostpointercapture", L),
    testimonialsSlider.addEventListener("keydown", (e) => {
      ("ArrowRight" === e.key &&
        (e.preventDefault(), d(o + 1, { animate: !t })),
        "ArrowLeft" === e.key &&
          (e.preventDefault(), d(o - 1, { animate: !t })));
    }));
  const S = testimonialsTrack.querySelector('svg[width="0"][height="0"]'),
    E = S?.querySelector("#testimonialStarGradient");
  if (E) {
    const e = testimonialsTrack.querySelectorAll(".testimonial__stars svg");
    let t = 0;
    e.forEach((e) => {
      e.querySelectorAll(
        'defs[data-nb-local="testimonialStarGradient"]',
      ).forEach((e) => e.remove());
      const o = document.createElementNS("http://www.w3.org/2000/svg", "defs");
      o.setAttribute("data-nb-local", "testimonialStarGradient");
      const r = E.cloneNode(!0),
        s = "testimonialStarGradient-local-" + t++;
      ((r.id = s),
        o.appendChild(r),
        e.insertBefore(o, e.firstChild),
        (e.style.stroke = `url(#${s})`),
        (e.style.fill = "none"));
    });
  }
}
...
});
{
  const heroTrack = document.querySelector(".hero__track"),
    heroDots = document.querySelectorAll(".hero__dot");
  if (heroTrack && heroDots.length > 1) {
    ...
    const go = (i) => {
        ((idx = ((i % count) + count) % count),
          (heroTrack.style.transform = "translateX(-" + 100 * idx + "%)"),
      ...
      play = () => {
        (clearInterval(autoplay),
          (autoplay = setInterval(() => go(idx + 1), 5e3)));
      },
      pause = () => clearInterval(autoplay);
    heroDots.forEach((d, i) =>
      d.addEventListener("click", () => {
        (go(i), play());
      }),
    );
    const heroTop = heroTrack.closest(".hero__top");
    (heroTop &&
      (heroTop.addEventListener("mouseenter", pause),
      heroTop.addEventListener("mouseleave", play),
      heroTop.addEventListener(
        "touchstart",
        (e) => {
          ((tx = e.touches[0].clientX), (swiping = !0), pause());
        },
        { passive: !0 },
      ),
      heroTop.addEventListener(
        ...
        (e) => {
          swiping &&
            ((swiping = !1),
            Math.abs(tx - e.changedTouches[0].clientX) > 50 &&
              go(idx + (tx - e.changedTouches[0].clientX > 0 ? 1 : -1)),
            play());
        },
        { passive: !0 },
      )),
